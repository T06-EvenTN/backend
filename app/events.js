    ...
    try {
        ...
        if (eventEndDate < eventStartDate) {
            return res
                ...
                .send({
                    message:
                        "event start date cannot be before event end date.",
                });
        }
            ...
            if (req.file) {
                ...
                if (event) {
                    if (eventMatchesCreator(event)) {
                        ...
                        const result = await cloudinary.uploader.upload(
                            ...
                            function (err, result) {
                                if (err) {
                                    ...
                                    console.log(
                                        `upload error for event ${eventId}`
                                    );
                                    console.log(err);
                        ...
                        res.status(403).send({
                            message: "user is not the event creator",
                        });
                } else
                    res.status(404).send({
                        message: "supplied eventId has no corresponding event",
                    });
            ...
            if (validReq) {
                if (!(req.user._id == validReq.eventCreatedBy)) {
                    return res
                        ...
                        .send({
                            message:
                                "User is not the same that created the event.",
                        });
                }
                if (
                    ...
                    return res
                        .status(400)
                        .send({ message: "Name must be a string." });
                }
                if (
                ...
                if (
                    req.body.eventStart &&
                    isNaN(Date.parse(req.body.eventStart))
                ) {
                    return res
                        .status(400)
                        .send({ message: "Invalid event start date." });
                ...
                if (
                    req.body.eventTag &&
                    !eventTags.includes(req.body.eventTag)
                ) {
                    return res.status(400).send({
                        message: `Invalid tag. Must be one of: ${eventTags.join(
                            ", "
                ...
                const newEventStart =
                    req.body.eventStart ?? validReq.eventStart;
                const newEventLength =
                    req.body.eventLength ?? validReq.eventLength;
        ...
        if (
            ...
            if (validReq) {
                if (!user.events.includes(id)) {
                    ...
                    res.status(200).send(
                        `increased attendance for event ${id}`
                    );
                } else {
                    await Event.findByIdAndUpdate(id, {
                        eventPresence: validReq.eventPresence - 1,
                    ...
                    res.status(200).send(
                        `decreased attendance for event ${id}`
                    );
                }
            } else res.status(404).send({ message: "event not found" });
        } else
            res.status(400).send({
                message: "user ID or event ID is not valid",
            });
    } catch (error) {
        res.status(500).send({ message: `internal error: ${error}` });
    }
